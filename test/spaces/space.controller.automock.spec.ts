...
import {
  ...
  I18nMock,
  PrismaMock,
} from 'test/fixture/mock/shared-mock';
...
describe('SpaceController (isolated)', () => {
  ...
  beforeEach(async () => {
    const moduleRef = await Test.createTestingModule({
      ...
      .useMocker((token) => {
        if (token === SpaceService) {
          return {
            create: jest.fn(),
            addManagers: jest.fn(),
            findPublicSpaces: jest.fn(),
            findSpacesByManagers: jest.fn(),
            findDetailPublicSpace: jest.fn(),
            update: jest.fn(),
            delete: jest.fn(),
          };
        }
  ...
  describe('addManagers', () => {
    ...
    it('should throw BadRequestException if spaceId is null', async () => {
      const addManagersSpy = jest
        .spyOn(service, 'addManagers')
        .mockRejectedValue(new TypeError('Validation Error'));
      await expect(
        controller.addManagers(
          CurrentUserExample,
  ...
  describe('update', () => {
    ...
    it('should throw Bad Request if spaceId is null', async () => {
      const spaceUpdateSpy = jest
        .spyOn(service, 'update')
        .mockRejectedValue(new BadRequestException('ParseIntPip Error'));
      await expect(
